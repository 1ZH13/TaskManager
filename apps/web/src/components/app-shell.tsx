...
export function AppShell({ children }: { children: ReactNode }) {
  const t = useTranslations('app'); const pathname = usePathname(); const searchParams = useSearchParams(); const [open, setOpen] = useState(false); const [collapsed, setCollapsed] = useState(false); const [confirming, setConfirming] = useState(false); const [resetNotice, setResetNotice] = useState(false); const { phId, phName, actor, setPhId, setActorId, reset } = useDemo(); const section = ['administrativa', 'operaciones', 'contabilidad'].find((candidate) => pathname.startsWith(`/${candidate}/`)); const projectBase = section ? `/${section}` : null; const projectId = searchParams.get('projectId'); const projectName = demoSeed.projects.find((project) => project.id === projectId)?.name ?? 'Proyecto'; const moduleLabel = section === 'administrativa' ? 'Gestión administrativa' : section === 'contabilidad' ? 'Gestión de contabilidad' : 'Gestión operativa'; const demoMode = process.env.NODE_ENV !== 'production';
